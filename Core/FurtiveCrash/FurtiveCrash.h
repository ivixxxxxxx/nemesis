...
namespace t6
{
    namespace offsets
    {
		...
		const auto CL_HandleRelayPacket =0x7EC480;
		const auto PartyHost_HandleJoinPartyRequest1 =0x7F94EC;
		const auto PartyHost_HandleJoinPartyRequest2 = 0x7F975C;

		const auto Session_GetCurrentSession =0x534520;
		const auto Party_GetPartyData =0x615150;
		const auto Party_FindMemberByXUID =0x51AD60;

	    const auto MSG_Init =  0x465E60;
        const auto MSG_WriteString =0x630CF0;
		const auto MSG_WriteLong = 0x473240;
		const auto MSG_WriteInt64 =0x558E30;
		const auto MSG_WriteByte =0x415940;
		const auto MSG_WriteShort =0x4677A0;
		const auto MSG_WriteFloat = 0x6B8A40;
		const auto MSG_WriteBit1 = 0x44EB30;
		const auto MSG_WriteBit0 =0x55C990;
		const auto MSG_ReadByte =0x6D4170;

		const auto Com_ControllerIndex_GetNetworkID = 0x4DC750;
	    const auto NET_OutOfBandData = 0x45C230;

		const auto CL_SendPeerData =0x51FA60;
		const auto CL_CanWeConnectToClient = 0x639CF0;

		const auto Live_GetXuid =0x4EA8C0;
    }
